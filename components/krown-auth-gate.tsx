...
export function KrownAuthGate() {
  ...
  function finishLogin(staff: StaffProfile, token: string, returnedDeviceId?: string | null) {
    localStorage.setItem('krown_session_token', token); localStorage.setItem('krown_staff_profile', JSON.stringify(staff)); sessionStorage.setItem('krown_active_session', 'true');
    if (returnedDeviceId) localStorage.setItem('krown_device_id', returnedDeviceId);
    window.location.reload();
  }
  function profileFromStaff(s: any): StaffProfile { return { id: s.id, name: s.name || 'Staff', email: s.email || '', role: s.role, branch: s.branch || 'Global HQ', assignedBranchId: s.assigned_branch_id || s.assignedBranchId || null, status: s.status || 'active', avatar: s.avatar }; }
  async function tryAdminPin(): Promise<boolean> { const response = await fetch('/api/auth/pin-login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pin }) }); const json = await response.json(); if (!response.ok || !json?.data?.staff || !json?.data?.token) return false; finishLogin(profileFromStaff(json.data.staff), json.data.token, json.data.deviceId); return true; }
  ...
  async function handlePinLogin() {
    ...
    try {
      ...
      if (!isActivated) { setError('This terminal is not registered. Ask the Super Admin to activate it with the one-time PIN or QR code.'); setMode('activate'); return; }
    ...
    } catch (e: any) { setError(e?.message || 'Unable to sign in securely.'); }
  ...
  async function activate() {
    ...
    setBusy(true); setError('');
    try { const data = await activateDevice(cleaned); setDeviceId(data.id); setActivationPin(''); setMode('pin'); }
  ...
  async function startScanner() {
    ...
    try {
      const Detector = (window as any).BarcodeDetector; const detector = new Detector({ formats: ['qr_code'] });
      ...
      scanTimerRef.current = window.setInterval(async () => { if (!videoRef.current || videoRef.current.readyState < 2) return; try { const results = await detector.detect(videoRef.current); const value = results?.[0]?.rawValue; if (value) { setActivationPin(value); stopScanner(); await activate(); } } catch { /* frame not decodable */ } }, 300);
