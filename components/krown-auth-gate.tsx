  ...
  async function startScanner() {
    ...
    try {
      ...
      scanTimerRef.current = window.setInterval(async () => {
        ...
        try { const results = await detector.detect(videoRef.current); const value = results?.[0]?.rawValue; if (value) { setActivationPin(value); stopScanner(); await activate(); } } catch { /* frame not decodable */ }
      }, 300);
