...
async function context(request: NextRequest) { return extractTenantContext(request); }
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ data: product });
  } catch (error: any) { return NextResponse.json({ error: error.message || 'Failed to update product' }, { status: 500 }); }
}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const { id } = await params; await deleteProduct(ctx, id); return NextResponse.json({ data: { success: true } }); }
  catch (error: any) { return NextResponse.json({ error: error.message || 'Failed to delete product' }, { status: 500 }); }
}
