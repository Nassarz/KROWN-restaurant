...
export function verifyTokenSync(token: string): TokenPayload | null {
  try {
    ...
    if (typeof payload.email !== 'string' || (payload.branch !== null && typeof payload.branch !== 'string')) return null;
...
export async function getSession(token: string): Promise<AuthResult> {
  ...
  // Platform administrators are stored separately from tenant staff. Never
  // attempt to resolve a platform identity through the tenant staff table.
  if (payload.role === 'super_admin' && payload.org === SUPER_ADMIN_ORG) {
    ...
    return { success: true, staff: { id: admin.id, name: admin.name, email: admin.email, role: 'super_admin', branch: '', assignedBranchId: null, organizationId: 'super-admin' } };
  }
...
export async function authenticateSuperAdmin(email: string, password: string): Promise<AuthResult> {
  ...
  return { success: true, token: await createToken({ sub: admin.id, org: SUPER_ADMIN_ORG, role: 'super_admin', branch: null, email: admin.email }), staff: { id: admin.id, name: admin.name, email: admin.email, role: 'super_admin', branch: '', assignedBranchId: null, organizationId: 'super-admin' } };
}
