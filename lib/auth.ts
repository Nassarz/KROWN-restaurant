...
export async function verifyToken(token: string): Promise<TokenPayload | null> {
  try {
    const { payload } = await jwtVerify(token, JWT_SECRET, { issuer: 'krown-pos', algorithms: ['HS256'] });
...
export interface AuthResult {
  success: boolean;
  token?: string;
  staff?: { id: string; name: string; email: string; role: string; branch: string; assignedBranchId: string | null; organizationId: string };
  error?: string;
}
...
export async function authenticateStaff(email: string, password: string): Promise<AuthResult> {
  ...
  const staffRows = await queryWithRetry(() => sql`
    SELECT id, name, email, role, branch, assigned_branch_id, organization_id,
           password_argon2, status
    FROM staff WHERE lower(email) = ${email.toLowerCase().trim()} LIMIT 1
  `);
  if (!staffRows.length) return { success: false, error: 'Invalid email or password' };
  const staff = staffRows[0] as any;
  if (staff.status !== 'active') return { success: false, error: 'Account is not active' };
  if (!staff.password_argon2 || !(await verifyPassword(staff.password_argon2, password))) {
...
export async function authenticateByPin(email: string, pin: string): Promise<AuthResult> {
  ...
  const staffRows = await sql`
    SELECT id, name, email, role, branch, assigned_branch_id, organization_id,
           pin_argon2, status FROM staff
    WHERE lower(email) = ${email.toLowerCase().trim()} LIMIT 1
  `;
  if (!staffRows.length) return { success: false, error: 'Invalid email or PIN' };
  const staff = staffRows[0] as any;
  if (staff.status !== 'active') return { success: false, error: 'Account is not active' };
  ...
  if (lockout.length) {
    ...
    const lockedUntil = lo.locked_until instanceof Date ? lo.locked_until.getTime() : Number(lo.locked_until || 0);
    if (lockedUntil > Date.now()) {
      return { success: false, error: `Account locked. Try again in ${Math.ceil((lockedUntil - Date.now()) / 60000)} minutes` };
    }
  }

  if (!staff.pin_argon2 || !(await verifyPassword(staff.pin_argon2, pin))) {
    ...
    await sql`
      ...
      ON CONFLICT (staff_id) DO UPDATE SET failed_attempts = EXCLUDED.failed_attempts, locked_until = EXCLUDED.locked_until
    `;
...
export async function getSession(token: string): Promise<AuthResult> {
  ...
  const rows = await sql`
    ...
    FROM staff WHERE id = ${payload.sub} LIMIT 1
  `;
  if (!rows.length) return { success: false, error: 'Staff not found or inactive' };
  const staff = rows[0] as any;
  if (staff.status !== 'active') return { success: false, error: 'Staff not found or inactive' };
  if (staff.organization_id !== payload.org || staff.role !== payload.role || (staff.assigned_branch_id || null) !== (payload.branch || null)) {
...
export async function authenticateSuperAdmin(email: string, password: string): Promise<AuthResult> {
  const sql = getSql();
  const rows = await sql`SELECT id, name, email, password_hash, is_active FROM super_admins WHERE lower(email) = ${email.toLowerCase().trim()} LIMIT 1`;
  ...
  const token = await createToken({ sub: admin.id, org: '00000000-0000-0000-0000-000000000000', role: 'super_admin', branch: null, email: admin.email });
  return { success: true, token, staff: { id: admin.id, name: admin.name, email: admin.email, role: 'super_admin', branch: '', assignedBranchId: null, organizationId: 'super-admin' } };
}
...
export async function getUserFromRequest(request: Request): Promise<TokenPayload | null> {
  ...
  const session = await getSession(decodeURIComponent(token));
  if (!session.success || !session.staff) return null;
