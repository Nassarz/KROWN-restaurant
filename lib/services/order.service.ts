...
export async function createOrder(
  ...
  try {
    ...
    await sql`
      ...
      VALUES (${id}, ${ctx.organizationId}, ${input.branchId || ctx.branchId}, ${input.tableNumber}, ${input.seat || null}, 'pending', ${JSON.stringify(processedItems)}, ${subtotal}, ${tax}, ${total}, ${input.companyId ? 'unpaid' : 'unpaid'}, ${input.companyId || null}, ${input.staffId || null}, ${input.companyName || null}, ${input.tin || null}, NOW(), NOW())
    `;
    ...
    const movements = await inventoryService.deductInventory(ctx, orderItems, id, input.branchId || ctx.branchId, null);
...
export async function payOrder(
  ...
  try {
    ...
    await sql`
      INSERT INTO accounting_ledger (id, organization_id, order_id, restaurant_id, type, amount, created_at)
      VALUES (${generateId()}, ${ctx.organizationId}, ${orderId}, ${order.branch_id || null}, 'PAYMENT', ${payment.amount}, NOW())
    `;
...
export async function splitPayment(
  ...
  try {
    ...
    for (const split of splits) {
      await sql`
        INSERT INTO accounting_ledger (id, organization_id, order_id, restaurant_id, type, amount, created_at)
        VALUES (${generateId()}, ${ctx.organizationId}, ${orderId}, ${order.branch_id || null}, 'PAYMENT', ${split.amount}, NOW())
      `;
...
export async function listOrders(
  ...
  if (branchId && startDate && endDate) {
    rows = await sql`SELECT * FROM orders WHERE organization_id = ${ctx.organizationId} AND restaurant_id = ${branchId} AND created_at >= ${startDate} AND created_at <= ${endDate} ORDER BY created_at DESC`;
  } else if (branchId) {
    ...
    rows = await sql`SELECT * FROM orders WHERE organization_id = ${ctx.organizationId} AND created_at >= ${startDate} AND created_at <= ${endDate} ORDER BY created_at DESC`;
  } else {
