...
export async function listStaff(ctx:TenantContext, branchId?:string):Promise<Staff[]> {
  ...
  if(ctx.isSuperAdmin){
    const rows=branchId&&branchId!=='all'
      ? await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE assigned_branch_id=${branchId} ORDER BY name ASC`
      : await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff ORDER BY name ASC`;
  ...
  const rows=effectiveBranch
    ? await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE organization_id=${ctx.organizationId} AND assigned_branch_id=${effectiveBranch} AND role!='super_admin' ORDER BY name ASC`
    : await sql`SELECT id,organization_id,name,email,phone,id_type,id_number,role,assigned_branch_id,status,avatar,created_at,updated_at FROM staff WHERE organization_id=${ctx.organizationId} AND role!='super_admin' ORDER BY name ASC`;
  return rows as Staff[];
}
...
export async function setPin(ctx:TenantContext,staffId:string,pin:string):Promise<void>{const sql=getSql();await setTenantContext(sql,ctx.organizationId);const existing=await sql`SELECT id FROM staff WHERE id=${staffId} AND organization_id=${ctx.organizationId}`;if(!existing.length)throw new Error('Staff not found');const hashedPin=await hashPassword(pin);await sql`UPDATE staff SET pin_code=NULL,pin_argon2=${hashedPin},updated_at=NOW() WHERE id=${staffId} AND organization_id=${ctx.organizationId}`;await logAudit(ctx.userId,'staff.set_pin',{staffId},ctx.organizationId,ctx.branchId);}
